...
- `InstanceMethodInterceptor+Single<Int|CGSize|CGRect|Object|OptionalObject>Void.swift`
...
Optional single-arg methods (for ObjC-bridgeable types like `UIView?` or `String?`) are routed through the
`OptionalObject` path and support `nil` arguments.
